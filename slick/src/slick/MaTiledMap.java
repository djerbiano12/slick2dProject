...
public class MaTiledMap extends TiledMap {
    ...
    private int nombrePieces; // nombre de pi√®ces √† placer dans le labyrinthe
    private int xPerso, yPerso, direction;  // param√®tres du personnage
    private int xPorte, yPorte; // param√®tres de la porte
    ...
    public MaTiledMap(String cheminCarte) throws SlickException {
        ...
        // On r√©cup√®re le nombre de pieces
        ...
        for (int objectID = 0; objectID < getObjectCount(0); objectID++) {
            // On r√©cup√®re les infos de la porte
    ...
    /**
     * D√©tecte une collision avec une tol√©rance epsilon
     */
    ...
    /**
     * D√©tecte s'il y a un mur aux coordon√©es x,y
    ...
    public boolean isMur(int x, int y) {
		 ...
		 * Aides:
		 * 
		 *  La map contient la mÈthode: 
		 *  Image getTileImage(x, y,indiceCouche)
		 * qui retourne l'image ‡ la position (x,y) de la couche indiquÈe (null sinon) 
		 * 
		 * et la mÈthode:
		 * int getLayerIndex(String couche) qui retourne l'indice de la couche passÈe en paramËtre
		 */
